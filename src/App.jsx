...
function QuizPage() {
  ...
  if (showResult) {
    return (
      <div className="quiz-page" style={{textAlign:'center'}}>
        ...
        <p style={{margin:'24px 0'}}>You've completed the Pokemon Type Quiz.<br/>Here's your score:</p>
        <div style={{display:'flex',justifyContent:'center',gap:24,marginBottom:24}}>
          <div style={{background:'#f8eaea',borderRadius:12,padding:'16px 24px'}}>
            <div style={{fontSize:18,fontWeight:'bold'}}>Final Score</div>
            <div style={{fontSize:24,color:'#e53935',fontWeight:'bold'}}>{Math.round((score/totalQuestions)*100)}%</div>
          </div>
          <div style={{background:'#f8eaea',borderRadius:12,padding:'16px 24px'}}>
            <div style={{fontSize:18,fontWeight:'bold'}}>Correct</div>
            <div style={{fontSize:24,color:'#43a047',fontWeight:'bold'}}>{score}/{totalQuestions}</div>
          </div>
        </div>
  ...
  return (
    <div className="quiz-page">
      <div style={{fontSize:14,marginBottom:8}}>Question {questionNum}/{totalQuestions}</div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:16}}>
        {choices.map(c => (
          <button key={c} className="primary-btn" style={{background: result && c===answer ? '#43a047' : undefined}} onClick={() => !result && handleChoice(c)} disabled={!!result}>{c.charAt(0).toUpperCase()+c.slice(1)}</button>
        ))}
      </div>
      {result && (
